...
def add_aircraft(aircraft_id, icao_code, registration, name, manufacturer, model):
    conn = database_connect()
    if conn is None:
        return None
    cursor = conn.cursor()
    try:
        ...
        VALUES (%s, %s, %s, %s, %s, %s)
        """
        cursor.execute(query, (aircraft_id, icao_code, registration, name, manufacturer, model))
        conn.commit()  # Commit the transaction
    except Exception as e:
        conn.rollback()  # Rollback if there's an error
...
def update_aircraft(aircraft_id, icao_code, registration, name, manufacturer, model):
    conn = database_connect()
    if conn is None:
        return None
    cursor = conn.cursor()
    try:
        query = """
        UPDATE aircraft
        SET icaocode = %s, aircraftregistration = %s, name = %s, manufacturer = %s, model = %s
        WHERE aircraftid = %s
        """
        cursor.execute(query, (icao_code, registration, name, manufacturer, model, aircraft_id))
        conn.commit()
    except Exception as e:
